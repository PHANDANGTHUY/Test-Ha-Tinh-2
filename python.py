...
def parse_info_from_text(text):
    ...
    info['tong_nhu_cau_von'] = safe_search(r"Nhu cầu vốn lưu động trên một vòng quay.*?đồng,([\d.,]+)", text, is_numeric=True, default=0)
    info['von_doi_ung'] = safe_search(r"Vốn đối ứng.*?đồng,([\d.,]+)", text, is_numeric=True, default=0)
    info['so_tien_vay'] = safe_search(r"Vốn vay Agribank.*?đồng,([\d.,]+)", text, is_numeric=True, default=0)
    info['lai_suat'] = safe_search(r"Lãi suất đề nghị:\s*([\d.,]+)\s*%/năm", text, is_numeric=True, default=5.0)
    info['thoi_gian_vay'] = safe_search(r"Thời hạn cho vay:\s*(\d+)\s*tháng", text, is_numeric=True, default=12)
